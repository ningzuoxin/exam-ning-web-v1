<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="limitedTime" label="限时" width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.limitedTime / 60 }} 分钟</span>
        </template>
      </el-table-column>
      <el-table-column prop="remainTimes" label="考试次数" width="120">
        <template slot-scope="scope">
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-if="
              scope.row.times === 0 ||
              scope.row.times - scope.row.resultTimes > 0
            "
            type="primary"
            size="mini"
